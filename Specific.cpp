...
List Mymodel_PXEM_NULL_individualcpp(
 ...
 // const bool verbose             // 调试输出 
	...
	double lambda2 = 1.0;        // lambda扩展参数初始值-固定
	...
	arma::vec zeros_col = arma::zeros<arma::vec>(g); // 全 0 列

 
	arma::mat Sigmaa = construct_block(D1, R, R.t(), D2); //拼接 Σa 
...
//	cout << Sigmaa.n_cols << Sigmaa.n_rows<< endl;
	...
	for (int iter = 1; iter < maxIter; iter ++ ){
      ...
      // 1. 更新后验协方差矩阵SigmaA(分块协方差阵) 
        if(Sigmaa.is_sympd()){
        Sigmaa_inv = arma::inv_sympd(Sigmaa);  // 对称正定矩阵专用 
         } else {
        cout << "***** Sigmaa PINV *********************************************" << endl;
        Sigmaa_inv =arma::inv(Sigmaa, arma::inv_opts::allow_approx);  // 伪逆/近似逆矩阵::inv(SigmaA_inv, arma::inv_opts::allow_approx)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        } 
        
        
      arma::mat Sigmaa_inv_1 = Sigmaa_inv.submat(0,  0, g-1, g-1);    // 左上块
      arma::mat Sigmaa_inv_2 = Sigmaa_inv.submat(0,  g, g-1, 2*g-1);  // 右上块
      arma::mat Sigmaa_inv_3 = Sigmaa_inv_2.t();                      // 左下块
      arma::mat Sigmaa_inv_4 = Sigmaa_inv.submat(g,  g, 2*g-1, 2*g-1);// 右下块 
...
//	cout << Sigmaa.n_cols << Sigmaa.n_rows<< endl;
	...
	int iter = 0;
	int final_iter;	
	arma::vec loglik = arma::zeros<arma::vec>(maxIter);               // 存储迭代似然值 
    double logdet1 = 0;
	double sign1 = 0;                       // 行列式计算辅助变量 
	double logdet2 = 0; 
	double sign2 = 0;
...
//	while(iter < 2 || (iter < maxIter && std::abs(loglik(iter - 1) - loglik(iter - 2)) >= tol)){
	...
	for (int iter = 1; iter < maxIter; iter ++ ){
      ...
      arma::mat Sigmaa_inv_1 = Sigmaa_inv.submat(0,  0, g-1, g-1);    // 左上块
      arma::mat Sigmaa_inv_2 = Sigmaa_inv.submat(0,  g, g-1, g+gNULL-1);  // 右上块
      arma::mat Sigmaa_inv_3 = Sigmaa_inv_2.t();                      // 左下块
      arma::mat Sigmaa_inv_4 = Sigmaa_inv.submat(g,  g, g+gNULL-1, g+gNULL-1);// 右下块 
      ...
      arma::mat SigmaA_inv = construct_block(SigmaA_inv_1, SigmaA_inv_2, SigmaA_inv_3, SigmaA_inv_4);
        ...
        SigmaA = arma::inv(SigmaA_inv, arma::inv_opts::allow_approx);  // 伪逆/近似逆矩阵::inv(SigmaA_inv, arma::inv_opts::allow_approx)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        } 
	  
      arma::mat SigmaA_1 = SigmaA.submat(0,  0, g-1, g-1);    // 左上块
      arma::mat SigmaA_2 = SigmaA.submat(0,  g, g-1, g+gNULL-1);  // 右上块
      arma::mat SigmaA_3 = SigmaA_2.t();                      // 左下块
      arma::mat SigmaA_4 = SigmaA.submat(g,  g, g+gNULL-1, g+gNULL-1);// 右下块 
...
List Mymodel_individualcppSpecific(
  ...
  output = Rcpp::List::create(
    ...
    _["SA1_loglkmaxs"] = SA1_loglkmaxs,      // 检验统计量 
    _["SA2_loglkmaxs"] = SA2_loglkmaxs  // P值 
    // ...其他输出参数 
  );
